		...
		<div class="container">
			...
			<div class="col-md-8 products-right">
				<div class="order">
					...
					<div>
            <p class="time">
              <span><?php echo date("Y-m-d", $orderarr[0]['time']);?></span>
              <span>订单号：<?php echo $_GET['code']; ?></span>
              <span><?php echo $orderarr[0]['statusname']?></span>
            </p>
            <table>
              <?php 
              $total = 0;
              foreach($orderarr as $oitem){ 
                $total +=  $oitem['num'] * $oitem['price'];
              ?>
              <tr>
                <td><img style="width:100px;" src="../public/uploads/thumb_<?php echo $oitem['img'];?>" alt=""></td>
                <td><?php echo $oitem['name'];?></td>
                <td><?php echo $oitem['num'];?></td>
                <td>￥<?php echo $oitem['price'];?></td>
              </tr>
            <?php }?>
            </table>
            <p class="total">
              <span>总金额：￥ <?php echo $total;?></span>
              <span>
              <?php if($orderarr[0]['statusname'] == '已完成'){ ?>
											<a href="./comment.php?code=<?php echo $_GET['code'];?>">请评价</a>
										<?php }else if($orderarr[0]['statusname'] == '已评价'){ ?>
											<span>订单已完成</span>
										<?php }else{ ?>
											<a href="./commit.php?code=<?php echo $_GET['code'];?>">确认收货</a>
										<?php } ?>
              </span>
            </p>
					</div>
